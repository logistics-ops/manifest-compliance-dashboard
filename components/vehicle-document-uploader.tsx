...
import {
  createVehicleDocumentUploadTargetAction,
  finalizeVehicleDocumentUploadAction,
} from "@/app/actions/vehicle-documents";
import { getDocumentMimeType, uploadStorageDocument, validateDocumentFile } from "@/lib/integrations/uploads";
import { createClient } from "@/lib/supabase/client";
import type { VehicleChecklistItem } from "@/lib/data/vehicles";

type VehicleDocumentUploaderProps = {
...
export function VehicleDocumentUploader({ item, canEdit }: VehicleDocumentUploaderProps) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const target = await createVehicleDocumentUploadTargetAction({
      ...
      const result = await finalizeVehicleDocumentUploadAction({
        equipmentId: item.equipmentId,
        documentName: item.name,
  ...
  return (
    <div className="grid grid-cols-[minmax(280px,1fr)_minmax(260px,0.75fr)] gap-3 rounded-md border border-white/10 bg-black/25 p-3 max-lg:grid-cols-1">
      <div className="grid gap-3">
          ...
          <span className="flex items-center justify-center gap-3 text-left max-sm:grid max-sm:justify-items-center max-sm:text-center">
            ...
            <span className="grid gap-1">
              <span className="text-sm font-extrabold text-white">
                {isUploading ? "Uploading vehicle document..." : hasFile ? "Replace document" : "Drop file or browse"}
              </span>
        ...
        <label className="grid gap-2 text-xs font-bold uppercase tracking-[0.18em] text-manifest-quiet">
          ...
          <textarea
            ...
            disabled={!canEdit}
            className="form-control min-h-20 resize-y disabled:cursor-not-allowed disabled:opacity-60"
            placeholder="Optional vehicle document notes."
          />
        </label>
